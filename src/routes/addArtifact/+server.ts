/** @format */
import { HTTPCodes, type Artifact, type ArtifactResponse, type WorkflowRun } from "$lib/utils/github.types.js";
...
function verifySignature(signature: string | null, body: string, secret: string | undefined): boolean {
    if (!signature || !secret)
        return false;
...
async function fetchArtifactUrls(workflow: WorkflowRun): Promise<object[] | null> {
    let response: object[] = [];
    
    const runResponse = await fetch(workflow.url, {
        headers: defaultHeaders
    });

    if (!runResponse.ok) {
        console.error(`artifact API responded with status: ${runResponse.status}`);
        return null;
    }

    const runData = await runResponse.json();
    
    if (!runData.artifacts_url)
        return null;
    ...
    const archive_download_url: string = artifactData.artifacts?.[0].archive_download_url;
